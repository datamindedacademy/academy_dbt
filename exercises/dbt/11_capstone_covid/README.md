# Exercise 11 — Capstone: migrate SQL to dbt

## TPC-H capstone for Databricks and Postgres

Use the existing TPC-H data for the course capstone.
The Covid reference below needs separate data and SQL changes on both backends.

The business question: which countries have the highest order revenue in the report interval?
Start from this query in the Databricks SQL editor:
...
For Postgres, remove the `samples.` prefixes.

1. Extend the project from exercises 1–10. Declare `nation` as a source.
2. Add `stg_nation`. Use the existing customer and orders staging models.
3. Add `int_orders_with_nation`. Join the staging models with `ref()`.
4. Use the report variables and macro from exercises 7–9 to filter the orders.
5. Add `revenue_per_nation`. Calculate the grouped result in this model.
6. Test that `nation_key` is unique and non-null in `revenue_per_nation`.
7. Add a singular test that checks for negative revenue.
8. Add descriptions. Run `dbt build`, then `dbt docs generate`.
9. Compare the result with the starting query. Check the dependency graph in the docs.

Fix the deliberate null in the example model before `dbt build`, as exercise 5 describes.

## Optional Covid reference

The remaining material uses Belgian Covid data and Snowflake SQL.
Neither Databricks nor the Postgres codespace contains these source tables.
Use this material as a design exercise until the data and SQL are available.
...
1. From the repository root, run `dbt init covid --skip-profile-setup`.
   Run `./create_profiles.sh`, then `cd covid`.
