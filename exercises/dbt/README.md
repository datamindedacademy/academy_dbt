...
order. Use the tips in each exercise if you need help.
...
| [11](11_capstone_covid/) | Capstone: TPC-H or Covid | everything combined |

## Connection details (used throughout)

Run `./create_profiles.sh` in the repository root once. It writes
`~/.dbt/profiles.yml` with targets for your configured databases.
Databricks is the default when its credentials exist:

| | `postgres` target | `databricks` target |
|---|---|---|
...
```bash
dbt run                        # Databricks when configured
dbt run --target postgres      # the local backup
